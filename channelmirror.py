...
class ChannelMirror(Cog):
    def __init__(self, bot):
        ...
        self.cursor.execute("CREATE TABLE IF NOT EXISTS channelmirror (source_guild_id BIGINT, source_channel_id BIGINT, destination_guild_id BIGINT, destination_channel_id BIGINT)")
        ...
        for mirror in self.cursor:
            if mirror[0] not in self.channelmirror_cache.keys():
                self.channelmirror_cache[mirror[0]] = []
            self.channelmirror_cache[mirror[0]].append(mirror[1])
        self.webhook_cache = {}
    ...
    async def create(self, ctx,
        ...
        destination_guild = await self.get_or_fetch_guild(ctx, destination_channel_guild_id)
        if not destination_guild:
            return
        destination_channel = await self.get_or_fetch_channel(ctx, destination_guild, destination_channel_id)
    ...
    async def delete_by_number(self, ctx,
        ...
        self.delete_webhook(mirror[2], mirror[3])
        self.db.commit()
        self.cursor.execute(
            "SELECT * FROM channelmirror WHERE source_guild_id = ? AND source_channel_id = ?",
    ...
    async def get_channelmirrors(self, guild_id):
        ...
        for mirror in mirrors:
            if mirror[0] == guild_id or mirror[2] == guild_id:
                source_guild = await self.get_or_fetch_guild(None, mirror[0])
                destination_guild = await self.get_or_fetch_guild(None, mirror[2])
                source_channel = await self.get_or_fetch_channel(None, source_guild, mirror[1])
                destination_channel = await self.get_or_fetch_channel(None, destination_guild, mirror[3])
    ...
    async def on_message_edit(self, message):
        if message.cached_message:
            if message.cached_message.author.bot:
                return
        source_guild = await self.get_or_fetch_guild(None, message.guild_id)
        if not source_guild:
            return
        source_channel = await self.get_or_fetch_channel(None, source_guild, message.channel_id)
        ...
        for messageid in messageids:
            destination_guild = await self.get_or_fetch_guild(None, messageid[3])
            if not destination_guild:
                continue
            destination_channel = await self.get_or_fetch_channel(None, destination_guild, messageid[4])
            if not destination_channel:
                continue
            webhook = await self.get_or_fetch_webhook(destination_guild.id, destination_channel.id)
            if not webhook:
    ...
    async def get_or_fetch_guild(self, ctx, guild_id):
        try:
    ...
    async def get_or_fetch_channel(self, ctx, guild, channel_id):
        try:
    ...
    async def get_or_fetch_webhook(self, guild_id, channel_id):
        ...
        destination_guild = await self.get_or_fetch_guild(None, guild_id)
        if not destination_guild:
            return
        destination_channel = await self.get_or_fetch_channel(None, destination_guild, channel_id)
        if not destination_channel:
            return
        webhooks = await destination_channel.webhooks()
        self.cursor.execute(
        ...
        webhook_id = self.cursor.fetchone()[0]
        for webhook in webhooks:
...
class NukeView2(View):
    ...
    async def yes(self, button: Button, interaction: Interaction):
        ...
        for webhook in self.cursor.fetchall():
            destination_guild = await self.get_or_fetch_guild(None, webhook[0])
            if not destination_guild:
                continue
            destination_channel = await self.get_or_fetch_channel(None, destination_guild, webhook[1])
            if not destination_channel:
                continue
            webhooks = await destination_channel.webhooks()
            for webhook in webhooks:
